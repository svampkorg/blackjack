...
def game_loop():
    ...
    while is_play_again:
        ...
        while (
            state.standing() != Standing.PlayerBlackjack
            and state.standing() != Standing.PlayerBust
            and is_hit
        ):
        ...
        if state.standing() != Standing.PlayerBust:
            while (
                state.standing() != Standing.DealerBlackjack
                and state.standing() != Standing.DealerBust
                and state.dealer_hand_value() < 17
            ):
                state.hit_dealer()
        ...
        is_play_again = (
            ...
            if state.player_wallet <= 0
                ...
                True
                if input("Do you want keep going? (enter 'y' to continue): ") == "y"
                else False
            )
        )
